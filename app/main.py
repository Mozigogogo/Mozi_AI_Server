...
def custom_openapi():
    ...
    openapi_schema = get_openapi(
        ...
        - 🔄 **流式响应**: 所有分析接口均采用SSE流式输出，实时展示分析过程
        - 🚀 **双模式**: 对话模式(简洁) vs 思考模式(深度)

        ### 快速开始
        1. 使用 `/chat/stream` 端点进行流式对话
        2. 使用 `/analyze/stream` 端点进行流式深度分析
        3. 使用 `/test/skill/answer/stream` 端点流式测试 Skill 系统

        ### 注意事项
        - 所有分析接口均为流式输出，采用 SSE (Server-Sent Events) 格式
        - 所有分析仅供参考，不构成投资建议
        - 加密货币市场具有高风险性
        - 请谨慎决策，自负风险
        """,
